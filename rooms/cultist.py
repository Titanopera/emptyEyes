        ...
        while(True):
            ...
            if(self.result[-1]=="None"):
                ...
                if(self.result[-1] == "question"):
                    text.textBox(self.display, 5, ["The Idealogue", "Because fDrEeSePdAoImR is greather than I", "I can entrust the truth to them"], ["I am one amongst many"], pygame.font.SysFont("stixgeneralbolita", 12, bold=False, italic=False), self.gameStateManager).run()                    
                ...
                if(self.result[-1] == "killIdealogue"):
                    text.textBox(self.display, 5, ["The Idealogue", "One moment they stare at you smugly the next they're on the floor dead."], pygame.font.SysFont("stixgeneralbolita", 12, bold=False, italic=False), self.gameStateManager).run()
                    text.textBox(self.display, 5, ["Alex", "Oh Above and Below she was frightening" "*I wipe the sweat off my brow and pick up the sword that clattered to the ground*"], pygame.font.SysFont("stixgeneralbolita", 12, bold=False, italic=False), self.gameStateManager).run()
                    self.gameStateManager.setCurrentState("fDrEeSePdAoImR")
                    return
                ...
                if(("pStone" in self.gameStateManager.result or "alchohol" in self.gameStateManager) and alex.skills["beg"] > 5):
                    alex.useBeg()
                    dialogue.append("4. (Pull Stone/Alchohol) I see you are looking for a gift, I wonder if this qould work for you")
                ...
                if("blackSuccess" in self.result or alex.skills["blackmail"] >=3):
                    alex.useBlackmail()
                ...
                elif("blackSuccess" in self.result or alex.skills["blackmail"] < 3):
                    alex.useBlackmail()
                    dialogue.append("7. Is a minion that understands that it's disposable a good minion, what would happen if I were to tell your boss")
